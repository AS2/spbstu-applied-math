...
from Deblur.DeblurCNN import DeblurCNN
from CustomMetrics.CustomMetrics import CustomAccurencyMetrics

# "Deblur" CNN manager class
# Input: Blured image
# Output: Focused image
# Idea: train CNN to contain blur operator and make 'blind' deconvolution of input data
class DeblurTrainer():
    # constructor
    def __init__(self):
        self.model = DeblurCNN()
        return
    ...
    # Method which provides loading model state from file
    def Train(self, epochs=10, learning_rate=0.01, isNeedToPlotLossGraph=True):
        # init optimizer
        ...
        for epoch in range(epochs):
            ...
            for i, (blured, tru_img) in enumerate(self.train_set):
                pred_img = self.model(blured)
            
                # count loss
                loss = torch.mean(torch.square(tru_img - pred_img))
            ...
            current_val_loss = 0
            for i, (blured, tru_img) in enumerate(self.test_set):
                pred_img = self.model(blured)

                loss = torch.mean(torch.square(pred_img - tru_img))
